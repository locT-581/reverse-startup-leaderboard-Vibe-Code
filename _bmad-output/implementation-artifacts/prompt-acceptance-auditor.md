...
## Specification Document (2-1-hostile-input-content-creation.md)
```markdown
---
story_id: 2.1
...
- [ ] Task 1: Database Schema & Relations Updates (AC: 6, 7)
  - [ ] Add the `comments` table to `apps/backend/db/schema.ts` with columns: `id`, `postId` (cascade onDelete), `content`, `wastedCalories`, `authorId` (cascade onDelete), `createdAt`, and `updatedAt`.
  - [ ] Configure `commentsRelations` in `schema.ts`.
  - [ ] Update `usersRelations` and `postsRelations` in `schema.ts` to support many-to-one and one-to-many comment relationships.
  - [ ] Run migration using `drizzle-kit push` (or backend script equivalent if configured) to sync the PostgreSQL database schema.
- [ ] Task 2: Backend Posts & Comments Modules (AC: 6, 7)
  - [ ] Create a `PostsModule` containing controllers and services to handle `POST /posts` (creating a post) and `POST /posts/:id/comments` (creating a comment).
  - [ ] Protect these endpoints with the existing `JwtAuthGuard` to extract the authenticated user.
  - [ ] In `PostsService`, use `calculateScoreHelper` to determine and store the score for both posts and comments.
  - [ ] Inject `LeaderboardService` in `PostsService` to call `broadcastUpdate()` whenever a new post or comment is successfully saved.
  - [ ] Update `LeaderboardService` to fetch and include comments (with author information) when constructing the leaderboard payload.
  - [ ] Register `PostsModule` in `apps/backend/src/app.module.ts`.
- [ ] Task 3: Frontend Server Actions & HostileInput Component (AC: 1, 2, 3, 4)
  - [ ] Create `apps/frontend/src/domains/leaderboard/components/HostileInput.tsx` to handle custom textareas/inputs with real-time jargon and length validations.
  - [ ] Implement style `HostileInput.module.css` using the project's CSS variables, showing the "Penalty Red" color theme for active validation errors.
  - [ ] Create frontend Server Actions in `apps/frontend/src/app/actions/posts.ts`: `actionCreatePost` and `actionCreateComment`.
- [ ] Task 4: Frontend UI Integration & Leaderboard Expansion (AC: 5)
  - [ ] Create `apps/frontend/src/domains/leaderboard/components/CreatePostModal.tsx` containing the post creation form using the `HostileInput` components.
  - [ ] Modify `apps/frontend/src/app/page.tsx` to display a "Propose Paradigm" button, which opens the `CreatePostModal` when clicked (requires authentication check).
  - [ ] Modify `apps/frontend/src/domains/leaderboard/components/LeaderboardGrid.tsx` to enable clicking a row to expand it.
  - [ ] Render the expanded view containing full content, comments list, and the comment form utilizing the `HostileInput` component.
- [ ] Task 5: Testing & Verification (AC: 1, 2, 3, 4, 5, 6, 7)
  - [ ] Create backend unit tests in `tests/unit/backend/posts/posts.service.spec.ts` verifying posts/comments insertion and correct score calculations.
  - [ ] Create Playwright E2E tests in `tests/e2e/hostile-input.spec.ts` validating interactive validations (jargon checks, length limits) and form submissions on the frontend.
